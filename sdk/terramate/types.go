...
// ResourcePolicyCheck summarizes policy analysis for a resource.
...
// Maps to ResourcesCollection in the OpenAPI spec.
type ResourcesListResponse struct {
	Resources       []Resource      `json:"resources"`
	PaginatedResult PaginatedResult `json:"paginated_result"`
}

// ResourcesListOptions represents options for listing resources.
// All filters from GET /v1/resources/{org_uuid} are supported.
type ResourcesListOptions struct {
	ListOptions
	// Status filters by resource status (ok, drifted, pending)
	Status []string
	// Technology filters by technology (e.g. terraform, opentofu)
	Technology []string
	// Provider filters by provider (e.g. aws, gcloud)
	Provider []string
	// Type filters by resource type (e.g. vpc, loadbalancer)
	Type []string
	// Repository filters by repository URLs
	Repository []string
	// Target filters by deployment target
	Target []string
	// ExtractedAccount filters by extracted account
	ExtractedAccount []string
	// StackID filters by stack ID (single stack)
	StackID int
	// IsArchived filters by stack archived status (true, false, or both)
	IsArchived []bool
	// PolicySeverity filters by policy check result (missing, none, passed, low, medium, high)
	PolicySeverity []string
	// Search matches stack title/description/path and resource extracted name/id/address (regex)
	Search string
	Sort   []string
}
